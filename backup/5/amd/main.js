...
//            console.log(name + ' module load success');
    ...
    async.waterfall(
        [
            ...
            function(next) {
                requirejs_addPopupModule('member_signin',function() {
                    next(null);
                });

            },
            function(next) {
                requirejs_addPopupModule('login',function() {
                    next(null);
                });

            },
            function(next) {
                requirejs_addPopupModule('wait',function() {
                    next(null);
                });

            },
            function(next) {
                requirejs_addPopupModule('alert_yesno',function() {
                    next(null);
                });

            },
            function(next) {
                requirejs_addPopupModule('input_form',function() {
                    next(null);
                });

            },
            function(next) {
                requirejs_addPopupModule('map',function() {
                    next(null);
                });

            },
            function(next) {
                requirejs_addPopupModule('detail',function() {
                    next(null);
                });

            },
            function(next) {
                requirejs_addPopupModule('camera',function() {
                    next(null);
                });
            ...
            function(next) {
                ...
                requirejs_addCardModule(
                    'list',//모듈이름
                    '#page-list .mk-content',//dom 에 삽입될 위치 샐랙터의 자식으로 들어감
                    function() {
                        next(null);
                    });
